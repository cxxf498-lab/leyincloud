...
export const RECOMMENDATION_RULES = [
  // === 1. 金融/政务（最高优先级） ===
  {
    ...
    recommend: () => ({
      cvm: { series: 'flagship', spec: '8核32G起', count: '2台起', reason: '金融/政务要求高安全、高可靠，旗舰型提供最佳性能和稳定性' },
      cbs: { type: 'extreme_ssd', size: '500GB起', reason: '金融级业务需要极高IOPS，极速型SSD提供最低延迟' },
      mysql: { arch: 'triple', spec: '8核32G + 500GB', reason: '核心数据不能丢，三节点架构提供金融级高可用' },
  ...
  // === 2. AI训练/渲染 ===
  {
    ...
    recommend: () => ({
      cvm: { series: 'gn_series', spec: 'GN10Xp（V100/T4/A100）', count: '1台起', reason: 'GPU异构计算是AI训练和渲染的唯一选择' },
      cbs: { type: 'extreme_ssd', size: '1TB起', reason: '训练数据读写量大，需要极高吞吐' },
      mysql: { arch: 'dual', spec: '4核16G + 200GB', reason: '训练任务管理数据库，双节点即可' },
  ...
  // === 3. 游戏/直播/社交（音视频+高并发） ===
  {
    ...
    recommend: (data) => ({
      cvm: { series: 'c_series', spec: '8核16G起', count: '2台起', reason: '高并发场景需要计算型实例，CPU性能优先' },
      cbs: { type: 'ssd', size: '500GB起', reason: 'SSD云盘保证数据库和应用IO性能' },
      mysql: { arch: 'cluster', spec: '8核32G + 500GB', reason: '高并发数据库需要集群版支持读写分离' },
  ...
  // === 4. 电商平台 ===
  {
    ...
    recommend: (data) => {
      ...
      return {
        cvm: {
          series: 's_series',
          spec: isLarge ? '8核16G' : '4核8G',
          count: isLarge ? '4台起' : '2台',
          reason: '电商业务均衡型实例最合适，性价比最高'
        },
        cbs: { type: isLarge ? 'enhanced_ssd' : 'ssd', size: '500GB起', reason: '商品图片、订单数据需要高可靠性存储' },
        mysql: {
          arch: isLarge ? 'cluster' : 'dual',
          spec: isLarge ? '8核32G + 500GB' : '4核16G + 200GB',
  ...
  // === 5. 高并发/计算密集型 ===
  {
    ...
    condition: (data) => {
      return data.cpuRequirement === '需要高主频计算（视频转码/科学计算）' ||
        data.cpuRequirement === '需要GPU（AI训练/渲染）' || // 但业务类型不是AI
        (parseInt(data.dau) > 50000)
    },
    recommend: () => ({
      cvm: { series: 'c_series', spec: '16核32G起', count: '2台起', reason: '计算密集型必须选择计算型C系列，CPU主频更高' },
      cbs: { type: 'enhanced_ssd', size: '500GB起', reason: '高IO场景需要增强型SSD保证吞吐' },
      mysql: { arch: 'cluster', spec: '8核32G + 500GB', reason: '高并发数据库需要集群版' },
      security: ['ddos_standard', 'waf_basic'],
  ...
  // === 6. 数据库/缓存为主 ===
  {
    ...
    recommend: () => ({
      cvm: { series: 'm_series', spec: '8核64G起', count: '1台起', reason: '大内存场景，内存型M系列性价比最高' },
      cbs: { type: 'extreme_ssd', size: '1TB起', reason: '数据库需要极低延迟的存储' },
      mysql: { arch: 'triple', spec: '16核128G + 1TB', reason: '数据库为核心业务，必须三节点最高可用' },
      security: ['ddos_standard', 'waf_basic'],
    ...
    recommend: (data) => {
      ...
      return {
        cvm: {
          ...
          spec: isLarge ? '8核16G' : '4核8G',
          count: isLarge ? '2台起' : '1台',
          reason: '在线教育需要稳定的计算和网络性能'
        },
        cbs: { type: 'ssd', size: '200GB起', reason: '课程视频和课件需要较好的IO性能' },
        mysql: { arch: 'dual', spec: '4核16G + 200GB', reason: '学员数据和课程数据需要高可用' },
        security: ['ddos_standard', 'waf_enterprise'],
